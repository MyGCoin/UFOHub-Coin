...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s ufohub-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  ufohub-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded ufohub transaction") + "\n" +
              "  ufohub-tx [options] -create [commands]   " + _("Create hex-encoded ufohub transaction") + "\n" +
              "\n";
